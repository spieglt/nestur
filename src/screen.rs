...
pub fn draw_pixel(buffer: &mut Vec<u8>, x: usize, y: usize, color: RGBColor) {
    ...
    for sdl_row_num in 0..SCALE_FACTOR {
        // looping over one-pixel tall rows up to SCALE_FACTOR
        ...
        for sdl_col_num in 0..SCALE_FACTOR {
            // for pixels up to SCALE_FACTOR, moving horizontally
...
pub fn draw_to_window(
    ...
    texture
        .update(None, buffer, 256 * 3 * SCALE_FACTOR)
        .map_err(|e| e.to_string())?;
